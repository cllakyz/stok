...
class helper
{
    ...
    // sef url
    static function sef($str, $options = array())
    {
    ...
    // durum çıktısı
    static function outputStatus($sonuc, $yes = NULL, $no = NULL)
    {
        if (!$yes) {
            $yes = gettext("İşlem başarılı");
        }
        if (!$no) {
            $no = gettext("Hata");
        }
        if ($sonuc > 0) {
            $cikti = array('status_code' => 100, 'status_text' => $yes);
        } else {
            $cikti = array('status_code' => 101, 'status_text' => $no);
        }
        return $cikti;
    }

    // sonuç listesi ve durum
    static function outputDataStatus($data = array(), $yes = NULL, $no = NULL)
    {
        if (!$yes) {
            $yes = gettext("İşlem başarılı");
        }
        ...
        if (!empty($data)) {
            $cikti = array('status_code' => 100, 'status_text' => $yes, 'data' => $data);
        } else {
            $cikti = array('status_code' => 101, 'status_text' => $no);
        }
        return $cikti;
