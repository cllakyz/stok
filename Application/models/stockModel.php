...
class stockModel extends model
{
    ...
    public function stockAdd($prd_id, $cus_id, $type, $qty, $price)
    {
        return DB::insert("INSERT INTO $this->table SET product_id = ?, customer_id = ?, action_type = ?, quantity = ?, price = ?, create_date = ?", array($prd_id, $cus_id, $type, $qty, $price, $this->zaman));
    }

    public function stockEdit($id, $prd_id, $cus_id, $type, $qty, $price)
    {
        return DB::exec("UPDATE $this->table SET product_id = ?, customer_id = ?, action_type = ?, quantity = ?, price = ?, update_date = ? WHERE id = ?", array($prd_id, $cus_id, $type, $qty, $price, $this->zaman, $id));
    }
    ...
    {
        return $this->getList("SELECT $this->table.*, $this->product_table.name AS product_name, $this->customer_table AS customer_name, $this->customer_table.surname AS customer_surname 
            FROM $this->table 
